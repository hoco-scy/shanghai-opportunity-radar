...
export async function runLiveSmoke() {
  const [buaaOut, iguopinOut, ncss] = await Promise.all([
    run(process.execPath, ["scripts/collect-buaa-discovery.mjs", "--city", "上海"]),
    run(process.execPath, ["scripts/collect-iguopin-discovery.mjs", "--city", "上海"]),
    collectNCSSDiscovery({ city: "上海" })
  ]);
