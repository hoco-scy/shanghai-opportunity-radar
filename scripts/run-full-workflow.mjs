...
async function probeOfficialEntry(source, checkedAt) {
  ...
  for (let index = 0; index < attemptCount && !usable; index += 1) {
    ...
    try {
      const response = await fetch(requestedUrl, { redirect: "follow", signal: AbortSignal.timeout(12000), headers: { "user-agent": "Mozilla/5.0" } });
      const body = (await response.text()).slice(0, 12000);
      const isSemantic404 = /(?:页面不存在|not found|error 404|访问出错)/i.test(body);
      if (response.ok && !isSemantic404) { usable = true; accessEvidence.push({ requestedUrl, finalUrl: response.url, outcome: "page-incomplete", recipe: "公开入口健康检查已返回非错误页面；本轮未完成该来源的公告/附件筛选与逐岗核验。" }); }
      else if (isSemantic404) { semantic404 = true; accessEvidence.push({ requestedUrl, outcome: "semantic-404", recipe: "公开入口返回明确的不存在或错误页。" }); }
      else accessEvidence.push({ requestedUrl, outcome: "network-error", recipe: `公开入口返回 HTTP ${response.status}，未取得可用于采集的页面。` });
    } catch (error) { accessEvidence.push({ requestedUrl, outcome: "network-error", recipe: `公开入口本轮无法连接：${error?.name || "fetch-error"}。` }); }
  }
...
function batchesOf(items) {
  const batchCount = Math.max(1, Math.ceil(items.length / 60)); const batchSize = Math.ceil(items.length / batchCount);
...
function semanticDecisions(manifest, details, { city, sourceId }) {
  ...
  const batches = batchesOf(details); const batchDecisions = manifest.batchDecisions || [];
  ...
  return details.map((detail) => {
    const batchIndex = batches.findIndex((batch) => batch.includes(detail)); const batchDecision = batchDecisions[batchIndex];
    ...
    const decision = { ...batchDecision, ...(overrides.get(detail.officialUrl) || {}), officialUrl: detail.officialUrl, fingerprint: fingerprint(detail) }; const required = ["officialUrl", "fingerprint", "decision", "reasonCode", "reason", "semanticBasis"];
    ...
    if (decision.decision === "accepted" && (!["重点关注", "需要确认"].includes(decision.matchLevel) || !Number.isInteger(decision.priority) || decision.priority < 0 || decision.priority > 100 || !decision.matchReason)) throw new Error(`拟发布岗位缺少人工判断的匹配等级、理由或优先级：${detail.officialUrl}`);
    return decision;
  });
}
...
async function main() {
  ...
  const existingJobs = new Map(opportunities.jobs.filter((job) => job.sourceId !== telecomSource.id).map((job) => [job.id, job])); let published = 0; let updated = 0;
