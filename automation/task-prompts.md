...
| 任务名称 | 时间与时区 | 目的 |
|---|---|---|
| 上海求职雷达 · 上午完整更新 | 每天 09:00，`Asia/Shanghai` | 独立完成一次全量同步 |
| 上海求职雷达 · 下午完整更新 | 每天 14:00，`Asia/Shanghai` | 独立完成一次全量同步 |
...
```text
执行一次独立、全量的上海求职雷达同步。无论当天是否已经运行过其他任务，本轮都必须完成 AUTOMATION.md 规定的全部来源和对象检查，不能把上一次运行结果当作本轮已完成。
...
```text
处理 source-plan.json 的 everyRunOfficial 全部官方来源和 everyRunDiscovery 全部发现来源、仍在有效期内的岗位/公告/预公告、距逐岗复查满 24 小时的记录，以及上轮失败或 deferred 项。上海岗位优先处理，但不能排除或跳过其他地区。优先使用官方招聘站自己的届别、地点、学历、岗位类别、发布时间等筛选，不能直接遍历未筛选的大型岗位全集。
